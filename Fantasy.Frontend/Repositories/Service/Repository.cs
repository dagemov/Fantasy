...
namespace Fantasy.Frontend.Repositories.Service;
...
public class Repository : IRepository
{
    ...
    private async Task<T> UnserializeAnswer<T>(HttpResponseMessage responseHttp)
    {
        ...
        */
        ...
        var apiResponse = JsonSerializer.Deserialize<ApiResponse<T>>(response, _jsonDefaultOptions);

        if (apiResponse != null && apiResponse.IsSuccesfuly)
        {
            return apiResponse.Result!;
        }

        // Manejar posibles errores o excepciones
        throw new InvalidOperationException("Error in API response");
    }
}